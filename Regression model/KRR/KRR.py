...
for target in target_names:
    ...
    best_model = krr_opt.best_estimator_
    ...
    plt.savefig(f"KRR_{filename}.png", dpi=450)
    plt.show()

# SHAP analysis after all models are trained
writer = pd.ExcelWriter("KRR_SHAP_values_all_targets.xlsx", engine="openpyxl")
summary_rows = []
...
for target in target_names:
    ...
    shap_train = explainer.shap_values(
        X_train_df,
        nsamples=100
    )
    ...
    plt.tight_layout()
    plt.savefig(f"SHAP_KRR_{safe_target}.png", dpi=450)
    plt.show()
...
out_sample.to_excel(
    "Out_of_sample_ABX3_KRR_predictions.xlsx",
    index=False
)

print()
print("Saved SHAP values to KRR_SHAP_values_all_targets.xlsx")
print("Saved out-of-sample predictions to Out_of_sample_ABX3_KRR_predictions.xlsx")