...
for target in target_names:
    ...
    best_model = etr_opt.best_estimator_
    ...
    plt.savefig(f"ETR_{filename}.png", dpi=450)
    plt.show()

# SHAP analysis after all models are trained
writer = pd.ExcelWriter("ETR_SHAP_values_all_targets.xlsx", engine="openpyxl")
summary_rows = []
...
for target in target_names:
    ...
    plt.tight_layout()
    plt.savefig(f"SHAP_ETR_{safe_target}.png", dpi=450)
    plt.show()
...
out_sample.to_excel(
    "Out_of_sample_ABX3_ETR_predictions.xlsx",
    index=False
)

print()
print("Saved SHAP values to ETR_SHAP_values_all_targets.xlsx")
print("Saved out-of-sample predictions to Out_of_sample_ABX3_ETR_predictions.xlsx")