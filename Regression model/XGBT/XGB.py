...
for target in target_names:
    ...
    best_model = xgb_opt.best_estimator_
    ...
    plt.savefig(f"XGB_{filename}.png", dpi=450)
    plt.show()

# SHAP analysis after all models are trained
writer = pd.ExcelWriter("XGB_SHAP_values_all_targets.xlsx", engine="openpyxl")
summary_rows = []
...
for target in target_names:
    ...
    plt.tight_layout()
    plt.savefig(f"SHAP_XGB_{safe_target}.png", dpi=450)
    plt.show()
...
out_sample.to_excel(
    "Out_of_sample_ABX3_XGB_predictions.xlsx",
    index=False
)

print()
print("Saved SHAP values to XGB_SHAP_values_all_targets.xlsx")
print("Saved out-of-sample predictions to Out_of_sample_ABX3_XGB_predictions.xlsx")