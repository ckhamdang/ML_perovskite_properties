...
for target in target_names:
    ...
    best_model = gbrt_opt.best_estimator_
    ...
    ax.scatter(y_train, Pred_train, color="#FA7070", label="Train",
               s=150, edgecolor="k", marker="o")

    ax.scatter(y_test, Pred_test, color="#615EFC", label="Test",
               s=150, edgecolor="k", marker="o")
    ...
    plt.savefig(f"GBRT_{filename}.png", dpi=450)
    plt.show()

# SHAP analysis
writer = pd.ExcelWriter("GBRT_SHAP_values_all_targets.xlsx", engine="openpyxl")
summary_rows = []
...
for target in target_names:
    ...
    X_train_df = pd.DataFrame(result["X_train"], columns=feature_names)
    X_test_df = pd.DataFrame(result["X_test"], columns=feature_names)
    ...
    shap_train_df = pd.DataFrame(shap_train, columns=[f"SHAP_{f}" for f in feature_names])
    shap_test_df = pd.DataFrame(shap_test, columns=[f"SHAP_{f}" for f in feature_names])

    train_result = pd.concat(
        [
            X_train_df.reset_index(drop=True),
            pd.Series(result["y_train"], name=f"Actual_{target}"),
            pd.Series(result["Pred_train"], name=f"Predicted_{target}"),
    ...
    test_result = pd.concat(
        [
            ...
            pd.Series(result["y_test"], name=f"Actual_{target}"),
            pd.Series(result["Pred_test"], name=f"Predicted_{target}"),
    ...
    shap_output = pd.concat([train_result, test_result], ignore_index=True)
    ...
    plt.tight_layout()
    plt.savefig(f"SHAP_GBRT_{safe_target}.png", dpi=450)
    plt.show()
...
out_sample.to_excel(
    "Out_of_sample_ABX3_GBRT_predictions.xlsx",
    index=False
)

print()
print("Saved SHAP values to GBRT_SHAP_values_all_targets.xlsx")
print("Saved out-of-sample predictions to Out_of_sample_ABX3_GBRT_predictions.xlsx")